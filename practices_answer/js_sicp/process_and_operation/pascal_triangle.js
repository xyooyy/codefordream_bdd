...
function pascal_triangle_element(n,m){return factorial(n)/(factorial(m)*factorial(n-m));}
function factorial(n){if (n == 1 || n == 0) return 1;return n*factorial(n-1);}
function array_2_string(array){var str_array = "";for (var n = 0; n != array.length; n++)
    {for(var m = 0; m !=  array[n].length; m++)
        {str_array += array[n][m].toString();if (m != array[n].length-1) str_array += " ";}str_array += "\n";}return str_array;}