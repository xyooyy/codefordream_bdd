var stepDef = function(){
    ...
    this.When(/^input (.+) in (.+) and submit it$/, function (answer,file_name,callback){
        ...
        this.browser.execute(function(file_name,file_id,answer_code){
            var parent = document.getElementById('practice_page');
            var child = document.getElementsByClassName('CodeMirror CodeMirror-wrap cm-s-blackboard mCustomScrollbar _mCS_2')[0];
